...
function click() {
  ...
  if (_combo >= 6) tone({ type: 'sine', freq: base * 1.5, dur: 0.06, vol: 0.04 });
}

function buy() {
  [880, 1100, 1320, 1760].forEach((f, i) => tone({ type: 'square', freq: f, t: i * 0.05, dur: 0.1, vol: 0.1 }));
  tone({ type: 'triangle', freq: 440, glideTo: 880, dur: 0.18, vol: 0.08 });
}

function buyClick() {
  [1760, 2093, 2637, 3136].forEach((f, i) => tone({ type: 'square', freq: f, t: i * 0.03, dur: 0.07, vol: 0.08 }));
  tone({ type: 'sine', freq: 2637, glideTo: 5274, dur: 0.22, vol: 0.05 });
}

function buyStorage() {
  [80, 100, 130, 165].forEach((f, i) => tone({ type: 'sine', freq: f, t: i * 0.09, dur: 0.4, vol: 0.16 }));
...
let playing = false, musicTimer = null, beat = 0, ownedRef = () => ({});
...
function scheduleBeat() {
  ...
  beat++;
  musicTimer = setTimeout(scheduleBeat, BEAT * 1000);
}
...
function stopMusic() { playing = false; if (musicTimer) clearTimeout(musicTimer); musicTimer = null; }
...
export const Audio8 = {
  ...
  click, buy, buyClick, buyStorage, storageWarn,
